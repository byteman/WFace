...
bool DBConfiguration::exist(const std::string& table) const
{
    int count = 0;
    try
    {
...
bool DBConfiguration::getJSONObjects(const std::string &key, Poco::Util::JSONConfiguration &jsonObj)
{
    Poco::StringTokenizer token(key,".");
    if(token.count() != 2) return false;
    ...
    std::string jsonstr = getString(key);
    ...
    {
        cw_error("getJsonObject error %s",e.displayText().c_str());
        return false;
    }
